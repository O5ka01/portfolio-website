...
export default function BlogPage() {
  ...
  return (
    <div className="bg-warm-beige min-h-screen">
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-6 pt-8 pb-12 sm:py-16">
        {/* Mobile-optimized tag filtering */}
        {allTags.length > 0 && (
          <div className="mb-6 sm:mb-8">
            <h2 className="text-lg sm:text-xl font-semibold text-dark-text mb-2 sm:mb-3">{t('blog.categories')}</h2>
            <div className="flex flex-wrap gap-x-2 gap-y-2">
              <motion.button 
                onClick={() => setSelectedTag(null)}
                className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm ${!selectedTag ? 'bg-accent-primary text-white' : 'bg-accent-primary/20 text-accent-tertiary'}`}
              ...
              {allTags.map(tag => (
                <motion.button 
                  ...
                  className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm ${selectedTag === tag ? 'bg-accent-primary text-white' : 'bg-accent-primary/20 text-accent-tertiary'}`}
                  whileHover={{ scale: 1.03 }}
                  whileTap={{ scale: 0.98 }}
                >
                  {tag}
